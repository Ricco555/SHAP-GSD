...
def main() -> None:
    ...
    lines = [
        ...
        "WHAT",
        "----",
        "Four attack classes are characterised by absent network norms rather than",
        "present attack signatures. Negative Fidelity+ means the explanation correctly",
        "identifies missing features — it is semantically valid, not an explainer failure.",
        "",
        "KEY FINDINGS",
        "------------",
    ]
    ...
    lines += [
        ...
        "PAPER FRAMING",
        "-------------",
        f"SHAP-GSD identifies {len(absence_dominant)} absence-dominant attack classes:",
